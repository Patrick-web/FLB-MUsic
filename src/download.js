...
console.log(type);
type = type
  .replace(/http:\/\/www\.deezer\.com\/|www\.deezer\.com\//gi, "")
  .split("/");
...
function download(id) {
  return request(format("http://www.deezer.com/track/%d", id))
    .then((htmlString) => {
      ...
      if (!fs.existsSync(trackInfos.ALB_TITLE)) {
        console.log(
          "Directory " + trackInfos.ALB_TITLE + "\x1b[32m created \x1b[0m"
        );
        fs.mkdirSync(trackInfos.ALB_TITLE);
      }
      const fileName = format(
        trackInfos.ALB_TITLE + "/%s - %s.mp3",
        trackInfos.ART_NAME,
        trackInfos.SNG_TITLE
      ).replace(/[|&;$%@"<>()+,]/g, ""); //Illegal characters

      console.log(
        "Downloading : \x1b[36m %s %s %s \x1b[0m",
        trackInfos.ART_NAME,
        "-",
        trackInfos.SNG_TITLE
      );
...
function getTrackUrl(trackInfos) {
  const fileFormat = trackInfos.FILESIZE_MP3_320
    ? 3
    : trackInfos.FILESIZE_MP3_256
    ? 5
    : 1;

  const step1 = [
    trackInfos.MD5_ORIGIN,
    fileFormat,
    trackInfos.SNG_ID,
    trackInfos.MEDIA_VERSION,
  ].join("¤");
...
function getBlowfishKey(trackInfos) {
  ...
  const idMd5 = crypto
    .createHash("md5")
    .update(trackInfos.SNG_ID, "ascii")
    .digest("hex");
  let bfKey = "";
...
function addId3Tags(trackInfos, filename) {
  const coverUrl = format(
    "http://e-cdn-images.deezer.com/images/cover/%s/500x500.jpg",
    trackInfos.ALB_PICTURE
  );
  ...
  try {
    return request({ url: coverUrl, encoding: null }).then((body) => {
      ...
      if (trackInfos.SNG_CONTRIBUTORS.featuring)
        TPE1 = trackInfos.SNG_CONTRIBUTORS.featuring;
      else if (trackInfos.SNG_CONTRIBUTORS.mainartist)
        TPE1 = trackInfos.SNG_CONTRIBUTORS.mainartist;
      else TPE1 = [trackInfos.ART_NAME];

      writer
        .setFrame("TIT2", trackInfos.SNG_TITLE)
        .setFrame("TPE1", TPE1)
